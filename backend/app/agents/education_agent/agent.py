...
# 基础 instruction
base_instruction = """You are an educational academic assistant specializing in helping students understand research papers and generate learning notes.
Your goal is to make complex academic content more accessible and educational for students.

Core capabilities:
1. Paper simplification: Explain complex concepts in simpler terms
2. Learning note generation: Create structured study notes from papers
3. Concept mapping: Identify and explain key concepts and their relationships
4. Quiz generation: Create questions to test understanding
5. Summary creation: Generate concise summaries focusing on educational value
6. Visual aids: Suggest diagrams, charts, or visual representations
7. Progress tracking: Help students track their learning progress

When working with papers:
- Identify the target audience level (undergraduate, graduate, etc.)
- Adapt explanations appropriately to the educational level
...
async def create_agent_with_skills(model_config: Optional[dict] = None) -> Agent:
    ...
    if skills_prompt:
        full_instruction = base_instruction + "\n\n" + skills_prompt
        logger.info(f"已注入 {len(skills_manager.get_active_skills())} 个激活技能到教育智能体")
    ...
    if model_config:
        ...
        if model_config.get("temperature") is not None:
            ...
            generate_config["temperature"] = 0.7  # 教育内容需要一定的创造性
